...
const getTotalPassengers = (data) => data.length;
...
const getSurvivorCount = (data) => data.filter((passenger) => passenger.fields.survived === 'Yes').length;
...
// wish my linter would stop getting upset at the formatting. i give up hope its fine :/
...
const getCasualtyCountForClass = (data, pclass) => {
  const casualtyCount = data.filter((passenger) => {
    return passenger.fields.pclass === pclass && passenger.fields.survived === 'No';
  });
  return casualtyCount.length;
};
...
const getMinAge = (data) => {
  return data.filter((passenger) => {
    return passenger.fields.age !== undefined;
  }).reduce((min, passenger) => {
    return passenger.fields.age < min ? passenger.fields.age : min;
  }, 1000);
};
...
const getMinFare = (data) => {
  return data.filter((passenger) => {
    ...
    return passenger.fields.fare < min ? passenger.fields.fare : min;
  }, 1000);
};
...
const getTotalFare = (data) => {
  return data.filter((passenger) => {
    return passenger.fields.fare !== undefined;
  }).reduce((total, passenger) => {
    return passenger.fields.fare + total;
  }, 0);
};
...
const getAverageFare = (data) => {
  const passengers = data.filter((passenger) => {
    return passenger.fields.fare !== undefined;
  });
...
const getMedianFare = (data) => {
  const sortPassengers = data.filter((passenger) => {
    return passenger.fields.fare !== undefined;
  }).sort((a, b) => {
    return a.fields.fare - b.fields.fare;
  });
  const midIndex = Math.floor(sortPassengers.length / 2);
  if (sortPassengers.length % 2 === 0) {
    return (sortPassengers[midIndex - 1].fields.fare + sortPassengers[midIndex].fields.fare) / 2;
  }
  return sortPassengers[midIndex].fields.fare;
};
...
const getAverageAge = (data) => {
  ...
  return passengers.reduce((total, passenger) => {
    return passenger.fields.age + total;
  }, 0) / passengers.length;
};
...
const getMedianAge = (data) => {
  ...
  const midIndex = Math.floor(sortPassengers.length / 2);
  if (sortPassengers.length % 2 === 0) {
    return (sortPassengers[midIndex - 1].fields.age + sortPassengers[midIndex].fields.age) / 2;
  }
  return sortPassengers[midIndex].fields.age;
};
...
const getAverageAgeByGender = (data, gender) => {
  const passengers = data.filter((passenger) => {
    return passenger.fields.age !== undefined && passenger.fields.sex === gender;
  });
  return passengers.reduce((total, passenger) => {
    return passenger.fields.age + total;
  }, 0) / passengers.length;
