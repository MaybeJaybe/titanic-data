...
const countAllProperty = (data, property) => {
  return data.reduce((total, passenger) => {
    if (total[passenger.fields[property]] === undefined) {
      total[passenger.fields[property]] = 1;
    } else {
      total[passenger.fields[property]] += 1;
    }
    return total;
...
const makeHistogram = (data, property, step) => {
  const filterValues = data.filter((passenger) => {
    return passenger.fields[property] !== undefined;
  });
  const bucket = filterValues.reduce((total, passenger) => {
    if (total[Math.floor(passenger.fields[property] / step)] === undefined) {
      total[Math.floor(passenger.fields[property] / step)] = 1;
    } else {
      total[Math.floor(passenger.fields[property] / step)] += 1;
    }
    return total;
  }, []);
  return Array.from(bucket, (value) => value || 0);
};
...
const normalizeProperty = (data, property) => {
  const filterValues = data.filter((passenger) => passenger.fields[property] !== undefined);
  const values = filterValues.map((passenger) => passenger.fields[property]);
...
const getUniqueValues = (data, property) => {
  ...
  const uniqueValues = values.reduce((total, value) => total.add(value), new Set());
